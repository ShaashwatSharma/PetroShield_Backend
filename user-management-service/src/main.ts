...
app.post('/create-admin', checkRole(['Super_Admin']), async (req: Request, res: Response) => {
  try {
    ...
    // First, check if org group exists at root
    ...
    // Then, create or get the /org/Admin group
...
app.post('/create-manager', checkRole(['Super_Admin', 'Admin']), async (req: Request, res: Response) => {
  try {
    ...
    const adminOrg = adminOrgGroup.split('/')[1]; // Example: /MyOrg/Admin

    // ✅ Check if provided org matches admin's org
    if (adminOrg !== org) {
...
//checked
app.post('/assign-driver', checkRole(['Admin', 'Super_Admin']), async (req: Request, res: Response) => {
  try {
...
app.get('/my-manager/:managerUsername', checkRole(['Admin']), async (req: Request, res: Response) => {
  try {
    ...
    // ✅ Fetch manager details from Keycloak
